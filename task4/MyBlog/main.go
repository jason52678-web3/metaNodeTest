...
func main() {
	...
	var strArgs string
	if len(os.Args) < 2 {
		fmt.Println("Missing argument of configfile.")
		strArgs = "./conf/config.yaml"
		//	return
	} else {
		strArgs = os.Args[1]
	}

	err := settings.Init(strArgs)
	//err := settings.Init()
	if err != nil {
	...
	//6.启动服务
	srv := &http.Server{
		Addr:    string(fmt.Sprintf(":%d", viper.GetInt("port"))),
		Handler: r,
	}
